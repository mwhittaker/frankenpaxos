...
object AggregatorMain extends App {
  ...
  implicit class OptionsWrapper[A](o: scopt.OptionDef[A, Flags]) {
    def optionAction(
        f: (A, AggregatorOptions) => AggregatorOptions
  ...
  val parser = new scopt.OptionParser[Flags]("") {
    ...
    // Options.
    opt[Int]("options.numShardCutsPerProposal")
      .optionAction((x, o) => o.copy(numShardCutsPerProposal = x))
    opt[java.time.Duration]("options.recoverPeriod")
      .optionAction((x, o) => o.copy(recoverPeriod = x))
    opt[java.time.Duration]("options.leaderInfoPeriod")
      .optionAction((x, o) => o.copy(leaderInfoPeriod = x))
  ...
  val aggregator = new Aggregator[NettyTcpTransport](
