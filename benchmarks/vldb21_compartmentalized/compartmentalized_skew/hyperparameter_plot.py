...
def main(args) -> None:
    ...
    for y_columns in ['throughput', 'latency']:
        plot(
            df=df[
                (df['workload_label'] == 'compartmentalized_sweep_1')
            ],
            ax=next(axes),
            grouping_columns=('num_proxy_leaders',),
            x_column='num_clients',
            y_columns=[y_columns],
            title=('95% reads, 6 replicas'),
        )
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--output',
                        type=str,
                        default='compartmentalized_skew_hyperparameter.pdf',
