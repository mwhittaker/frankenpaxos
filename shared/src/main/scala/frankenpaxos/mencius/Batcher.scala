...
class BatcherMetrics(collectors: Collectors) {
  val requestsTotal: Counter = collectors.counter
    .build()
    .name("mencius_batcher_requests_total")
    .labelNames("type")
  ...
  val requestsLatency: Summary = collectors.summary
    .build()
    .name("mencius_batcher_requests_latency")
    .labelNames("type")
...
class Batcher[Transport <: frankenpaxos.Transport[Transport]](
  ...
  // batcher's best guess. The leader associated with this round can be
  // computed using `roundSystem`. The clients need to know who the leader is
  // because they need to know where to send their commands.
  @JSExport
  ...
  // When a batcher sends to a node that is not the leader, the node returns
  // the batch and tells the batcher that it's not the leader. The batcher then
  // sends LeaderInfoRequestBatcher messages to all the leaders trying to
  // figure out who the leader actually is. In the meantime, it buffers up
  // these rejected batches to be resent.
  @JSExport
  ...
  private def handleLeaderInfo(
    ...
    // Update our round.
    val oldRound = round(leaderInfo.leaderGroupIndex)
    val newRound = leaderInfo.round
    round(leaderInfo.leaderGroupIndex) = newRound
    ...
    if (roundSystem(leaderInfo.leaderGroupIndex).leader(oldRound) !=
          roundSystem(leaderInfo.leaderGroupIndex).leader(newRound)) {
      val leader = leaders(leaderInfo.leaderGroupIndex)(
        roundSystem(leaderInfo.leaderGroupIndex).leader(newRound)
      )
