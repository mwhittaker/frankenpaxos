...
class Replica[Transport <: frankenpaxos.Transport[Transport]](
  ...
  private def checkSameDependencies(
      ...
      numSameNeeded: Int,
      total: Int
  ): Tuple2[Boolean, Option[Seq[Instance]]] = {
  ...
  private def handlePreAcceptOk(
    ...
    if (value.commandInstance.leaderIndex == index) {
      ...
      if (preacceptResponses
        ...
        val depsTuple: Tuple2[Boolean, Option[Seq[Instance]]] =
          checkSameDependencies(
            preacceptResponses.getOrElse(value.commandInstance,
                                         mutable.ListBuffer.empty),
            N - 2,
            N
      ...
      if (state.nonEmpty) {
        ...
        for (edge <- state.get.dependencies) {
          ...
          if (edgeState.nonEmpty) {
            edgeList.append(
              (edgeState.get.command, edgeState.get.sequenceNumber)
            )
          }
        }
