...
object ReadBatcherMain extends App {
  case class Flags(
      // Basic flags.
      index: Int = -1,
  ...
  implicit class OptionsWrapper[A](o: scopt.OptionDef[A, Flags]) {
    def optionAction(
        f: (A, ReadBatcherOptions) => ReadBatcherOptions
  ...
  val parser = new scopt.OptionParser[Flags]("") {
    ...
    // Options.
    opt[ReadBatchingScheme]("options.readBatchingScheme")
      .optionAction((x, o) => o.copy(readBatchingScheme = x))
  ...
  // Construct ReadBatcher.
  val logger = new PrintLogger(flags.logLevel)
  val config = ConfigUtil.fromFile(flags.configFile.getAbsolutePath())
  val readBatcher = new ReadBatcher[NettyTcpTransport](
    address = config.readBatcherAddresses(flags.index),
