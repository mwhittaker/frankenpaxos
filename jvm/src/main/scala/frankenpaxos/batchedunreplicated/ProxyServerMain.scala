...
object ProxyServerMain extends App {
  case class Flags(
      // Basic flags.
      index: Int = -1,
  ...
  implicit class OptionsWrapper[A](o: scopt.OptionDef[A, Flags]) {
    def optionAction(
        f: (A, ProxyServerOptions) => ProxyServerOptions
  ...
  // Construct proxyServer.
  val logger = new PrintLogger(flags.logLevel)
  val config = ConfigUtil.fromFile(flags.configFile.getAbsolutePath())
  val proxyServer = new ProxyServer[NettyTcpTransport](
    address = config.proxyServerAddresses(flags.index),
