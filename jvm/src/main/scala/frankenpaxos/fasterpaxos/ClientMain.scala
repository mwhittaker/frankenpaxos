...
import frankenpaxos.UniformSingleKeyWorkload
...
object ClientMain extends App {
  case class Flags(
      ...
      workload: Workload = new UniformSingleKeyWorkload(1, 1, 0),
      // Options.
  ...
  // any stats.
  def warmupRun(pseudonym: Int, workload: Workload): Future[Unit] = {
    implicit val context = transport.executionContext
    val future = client.write(pseudonym, workload.get())
  ...
  def run(pseudonym: Int, workload: Workload): Future[Unit] = {
    implicit val context = transport.executionContext
    BenchmarkUtil
      .timed(() => client.write(pseudonym, workload.get()))
      .transformWith({
        case scala.util.Failure(_) =>
          logger.debug("Client write failed.")
          Future.successful(())
  ...
  implicit val context = transport.executionContext
  val warmupFutures = for (pseudonym <- 0 until flags.numWarmupClients) yield {
  ...
  val futures = for (pseudonym <- flags.numWarmupClients until
                       flags.numWarmupClients + flags.numClients) yield {
    BenchmarkUtil.runFor(() => run(pseudonym, flags.workload), flags.duration)
  }
