...
class MultiPaxosLeaderActor[Transport <: zeno.Transport[Transport]](
  ...
  config: MultiPaxosConfig[Transport]
    ...
    private var proposals: scala.collection.mutable.Map[Int, String] = scala.collection.mutable.Map()

    // For each commander spawned keep track of the acceptors who have not responded
    private var waitForCommander: scala.collection.mutable.Map[CommanderId, Set[Transport#Address]] = scala.collection.mutable.Map()

    // Keep track of acceptors who have not responded to Phase 1a requests
    private var waitForScout: Set[Transport#Address] = config.acceptorAddresses.toSet
    ...
    : Seq[TypedActorClient[Transport, MultiPaxosAcceptorActor[Transport]]] =
      ...
      private def scoutProcess(): Unit = {
        for (acceptor <- acceptors.toIterator) {
          acceptor.send(MultiPaxosAcceptorInbound().withPhase1A(
      ...
      private def commanderProcess(slotOne: Integer, commandOne: String): Unit = {
        for (acceptor <- acceptors.toIterator) {
          ...
          acceptor.send(MultiPaxosAcceptorInbound().withPhase2A(
            MultiPaxosPhase2a(leaderId = 0, proposal = ProposedValue(
              ballot = ballotNumber, slot = slotOne, command = commandOne))
          ))
        }
      }
      ...
      private def handlePhase1b(src: Transport#Address, request: MultiPaxosPhase1b): Unit = {
        ...
        if (request.ballot == ballotNumber) {
          ...
          if (waitForScout.size < (acceptors.size / 2)) {
            leader.send(MultiPaxosLeaderInbound().withAdopted(
              Adopted(ballot = ballotNumber, proposals = scoutProposalValues.toSeq)
            ))
          }
        } else {
          leader.send(MultiPaxosLeaderInbound().withPreempted(
      ...
      private def handlePhase2b(src: Transport#Address, request: MultiPaxosPhase2b): Unit = {
        if (request.ballot == ballotNumber) {
          val commanderId: CommanderId = CommanderId(request.phase2AProposal.slot, request.phase2AProposal.command)
          if (waitForCommander.get(commanderId).isEmpty) {
            waitForCommander.put(commanderId, config.acceptorAddresses.toSet)
          }

          var acceptorSet: Set[Transport#Address] = waitForCommander.get(commanderId).get
          acceptorSet -= src
          ...
          if (acceptorSet.size < (acceptors.size / 2)) {
            for (replica <- replicas) {
              ...
              replica.send(MultiPaxosReplicaInbound().withDecision(
                Decision(slot = request.phase2AProposal.slot, command = request.phase2AProposal.command)
              ))
            }
          }
        } else {
