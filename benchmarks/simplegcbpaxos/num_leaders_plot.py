...
def plot(df: pd.DataFrame, ax, column: str, pretty_column: str) -> None:
    for ((f, num_leaders), group) in df.groupby(['f', 'num_leaders']):
        stats = group.groupby(['num_clients'])[column].agg([np.mean, np.std])
        mean = stats['mean']
        std = stats['std'].fillna(0)

        line = ax.plot(mean, '.-', label=f'f={f}, num_leaders={num_leaders}')[0]
...
if __name__ == '__main__':
    parser = parser_util.get_plot_parser('simplegcbpaxos_num_leaders.pdf')
