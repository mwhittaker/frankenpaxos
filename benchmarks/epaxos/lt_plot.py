...
def main(args) -> None:
    ...
    fig, ax = plt.subplots(num_plots, 1,
                           figsize=(scale * 6.4, scale * num_plots * 4.8))
    ax_iter = iter(ax)
    line_group_by = [
        'f',
        'replica_dependency_graph',
        'replica_options.thrifty_system',
    ]
    plot_vs_num_clients(df, next(ax_iter), line_group_by,
         'Throughput', 'latency.median_ms', 'Median latency (ms)')
    plot_vs_num_clients(df, next(ax_iter), line_group_by,
         'Latency', 'stop_throughput_1s.p90',
         'P90 throughput (1 second windows)')
    plot_latency_throughput(df, next(ax_iter), line_group_by,
