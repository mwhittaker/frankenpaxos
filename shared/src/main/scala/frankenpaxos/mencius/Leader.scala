...
class LeaderMetrics(collectors: Collectors) {
  val requestsTotal: Counter = collectors.counter
    .build()
    .name("mencius_leader_requests_total")
    .labelNames("type")
  ...
  val requestsLatency: Summary = collectors.summary
    .build()
    .name("mencius_leader_requests_latency")
    .labelNames("type")
...
class Leader[Transport <: frankenpaxos.Transport[Transport]](
  ...
  @JSExportAll
  case class Phase1(
      // `phase1bs` has one entry per acceptor group. Every entry records
      // Phase1b responses.
      phase1bs: mutable.Buffer[mutable.Map[AcceptorIndex, Phase1b]],
      // If a leader receives a ClientRequestBatch during phase 1, it buffers
