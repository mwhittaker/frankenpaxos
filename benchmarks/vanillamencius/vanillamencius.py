...
class ServerOptions(NamedTuple):
    ...
    resend_phase1as_period: datetime.timedelta = \
        datetime.timedelta(seconds=1)
    flush_skip_slots_period: datetime.timedelta = \
        datetime.timedelta(milliseconds=100)
    revoke_min_period: datetime.timedelta = \
        datetime.timedelta(seconds=5)
    revoke_max_period: datetime.timedelta = \
...
class VanillaMenciusOutput(NamedTuple):
    output: benchmark.RecorderOutput


Output = VanillaMenciusOutput


# Networks #####################################################################
class VanillaMenciusNet:
...
# Suite ########################################################################
class VanillaMenciusSuite(benchmark.Suite[Input, Output]):
    ...
    def run_benchmark(self,
        ...
        # Write config file.
        net = VanillaMenciusNet(self._cluster, input)
        config = net.config()
        ...
        for (i, server) in enumerate(net.placement().servers):
            p = bench.popen(
                ...
                cmd=java(input.server_jvm_heap_size) + [
                    ...
                    'frankenpaxos.vanillamencius.ServerMain',
                    '--index',
                    str(i),
                    '--config',
                    config_filename,
                    ...
                    '{}s'.format(input.server_options
                                      .flush_skip_slots_period
                                      .total_seconds()),
                    '--options.revokeMinPeriod',
                    '{}s'.format(input.server_options
                                      .revoke_min_period
                                      .total_seconds()),
        ...
        for (i, client) in enumerate(net.placement().clients):
            p = bench.popen(
                ...
                cmd=java(input.client_jvm_heap_size) + [
                    '-cp',
                    os.path.abspath(args['jar']),
                    'frankenpaxos.vanillamencius.ClientMain',
                    '--host',
                    client.host.ip(),
                    '--port',
                    str(client.port),
        ...
        return VanillaMenciusOutput(output = output)
