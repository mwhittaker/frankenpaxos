...
def main(args) -> None:
    class ChaosSuite(MatchmakerMultiPaxosSuite):
        ...
        def inputs(self) -> Collection[Input]:
            return ([
                Input(
                    ...
                    driver_workload = driver_workload.Chaos(
                        leader_change_warmup_delay_ms = 5000,
                        leader_change_warmup_period_ms = 1000,
                        leader_change_warmup_num = 5,
                        reconfiguration_warmup_delay_ms = 10 * 1000,
                        reconfiguration_warmup_period_ms = 1000,
                        reconfiguration_warmup_num = 5,
                        matchmaker_reconfiguration_warmup_delay_ms = 15 * 1000,
                        matchmaker_reconfiguration_warmup_period_ms = 1000,
                        matchmaker_reconfiguration_warmup_num = 5,
                        leader_failure_delay_ms = 30 * 1000,
                        acceptor_failure_delay_ms = 30 * 1000,
                        matchmaker_failure_delay_ms = 30 * 1000,
                        acceptor_recover_delay_ms = 40 * 1000,
                        matchmaker_recover_delay_ms = 45 * 1000,
                    ),
    ...
    with benchmark.SuiteDirectory(args.suite_directory,
                                  'matchmaker_multipaxos_chaos') as dir:
        suite.run_suite(dir)
