...
def plot(n1: pd.DataFrame,
    ...
    failure = datetime.datetime(
        2020, 6, 30, hour=19, minute=45, second=22, microsecond=487000)
    leader_recover = datetime.datetime(
        2020, 6, 30, hour=19, minute=45, second=26, microsecond=487000)
    acceptor_recover = datetime.datetime(
        2020, 6, 30, hour=19, minute=45, second=32, microsecond=488000)
    matchmaker_recover = datetime.datetime(
        2020, 6, 30, hour=19, minute=45, second=37, microsecond=483000)

    for axes in ax:
        axes.axvline(x=origin + (failure - naive_start_time), color='red', ls='--')
        axes.axvline(x=origin + (leader_recover - naive_start_time), color='black')
        axes.axvline(x=origin + (acceptor_recover - naive_start_time), color='black')
        axes.axvline(x=origin + (matchmaker_recover - naive_start_time), color='black')
...
def main(args) -> None:
    ...
    (n4, _) = read_data(args.n4, args.drop_head, args.drop_tail,
                          nudge=datetime.timedelta(milliseconds=0))
