...
object AcceptorMain extends App {
  case class Flags(
      // Basic flags.
      leaderGroupIndex: Int = -1,
      acceptorGroupIndex: Int = -1,
      index: Int = -1,
  ...
  val parser = new scopt.OptionParser[Flags]("") {
    ...
    // Basic flags.
    opt[Int]("leader_group_index")
      .required()
      .action((x, f) => f.copy(leaderGroupIndex = x))
    opt[Int]("acceptor_group_index")
      .required()
      .action((x, f) => f.copy(acceptorGroupIndex = x))
  ...
  val acceptor = new Acceptor[NettyTcpTransport](
    address = config.acceptorAddresses(flags.leaderGroupIndex)(
      flags.acceptorGroupIndex
    )(flags.index),
