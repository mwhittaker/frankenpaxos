...
class Acceptor[Transport <: frankenpaxos.Transport[Transport]](
  ...
  protected var bufferedProposeRequests =
    mutable.Buffer[(Long, Transport#Address, Proposal)]()
  ...
  case class VVCommand(uniqueId: UniqueId) extends VoteValue
  ...
  private def handleProposeRequest(
    ...
    if (options.waitPeriod == java.time.Duration.ofSeconds(0) &&
      ...
      val t = (System.nanoTime(), src, proposal)
  ...
  private def processProposeRequest(
    ...
    log.get(nextSlot) match {
      case Some(Entry(voteRound, _, Some(r))) if r == round && voteRound < r =>
        ...
        log.put(nextSlot, Entry(r, VVCommand(proposal.uniqueId), None))
        val phase2b =
          Phase2b(acceptorId = acceptorId, slot = nextSlot, round = round).withUniqueId(proposal.uniqueId)
        nextSlot += 1
        Some(phase2b)
  ...
  private def processPhase2a(phase2a: Phase2a): Option[Phase2b] = {
    ...
    if (phase2a.round == voteRound) {
      ...
      return voteValue match {
        case VVCommand(uniqueId: UniqueId) =>
          Some(phase2b.withUniqueId(uniqueId))
        case VVNoop =>
    ...
    return phase2a.value match {
      case Phase2a.Value.UniqueId(uniqueId) =>
        log.put(phase2a.slot, Entry(round, VVCommand(uniqueId), None))
