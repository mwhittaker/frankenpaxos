...
class DependencyGraphTest extends FlatSpec with Matchers with PropertyChecks {
  ...
  "A ZigzagTarjanDependencyGraph" should "execute no commands correctly" in {
    val graph = zigzag(
      ZigzagTarjanDependencyGraphOptions(verticesGrowSize = 10,
                                         garbageCollectEveryNCommands = 10)
    )
    graph.executeByComponent(None) shouldBe (Seq(), Set((0, 0)))
  }
  ...
  it should "execute one command correctly" in {
    ...
    graph.executeByComponent(None) shouldBe (Seq(Seq((0, 0))), Set((1, 0)))
  }
  ...
  it should "execute a chain of commands correctly" in {
    ...
    graph.executeByComponent(None) shouldBe (Seq(Seq((0, 0)),
                                                 Seq((1, 0)),
                                                 Seq((2, 0)),
                                                 Seq((0, 1)),
                                                 Seq((1, 1)),
  ...
  it should "execute a forward edge with gap correctly" in {
    ...
    graph.executeByComponent(None) shouldBe
      (Seq(Seq((2, 0)), Seq((0, 0))), Set((1, 0)))

    graph.commit((1, 0), 0, new FakeCompactSet(Set()))
    graph.commit((0, 1), 1, new FakeCompactSet(Set()))
    graph.executeByComponent(None) shouldBe
      (Seq(Seq((1, 0)), Seq((0, 1))), Set((1, 1)))
  }
  ...
  it should "garbage collect correctly" in {
    ...
    graph.executeByComponent(None) shouldBe
      (Seq(Seq((0, 0)), Seq((1, 0)), Seq((2, 0))), Set((0, 1)))
  ...
  it should "handle unmet dep correctly" in {
    ...
    graph.executeByComponent(None) shouldBe (Seq(), Set((0, 1)))
    graph.commit((0, 1), 0, new FakeCompactSet(Set((0, 2))))
    graph.executeByComponent(None) shouldBe (Seq(), Set((0, 2)))
    graph.commit((0, 2), 0, new FakeCompactSet(Set((0, 3))))
    graph.executeByComponent(None) shouldBe (Seq(), Set((0, 3)))
    graph.commit((0, 3), 0, new FakeCompactSet(Set()))
    graph.executeByComponent(None) shouldBe
      (Seq(Seq((0, 3)), Seq((0, 2)), Seq((0, 1)), Seq((0, 0))), Set((1, 0)))
  }
