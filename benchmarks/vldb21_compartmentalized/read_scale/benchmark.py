...
def r(w: int, n: int) -> float:
    """
    r(w, n) returns the read ratio needed to have w writers given n nodes.
    """
    return 1 - (w / n)
...
def main(args) -> None:
    ...
    suite = Suite()
    with benchmark.SuiteDirectory(args.suite_directory, 'read_scale') as dir:
        suite.run_suite(dir)
