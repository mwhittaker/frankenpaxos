...
object ServerMain extends App {
  ...
  implicit class HeartbeatOptionsWrapper[A](o: scopt.OptionDef[A, Flags]) {
    def heartbeatOptionAction(
        ...
        (x, flags) =>
          flags.copy(
            options = flags.options
              .copy(heartbeatOptions = f(x, flags.options.heartbeatOptions))
          )
      )
  }
  ...
  val parser = new scopt.OptionParser[Flags]("") {
    ...
    // Options.
    opt[Boolean]("options.ackNoopsWithCommands")
      .optionAction((x, o) => o.copy(ackNoopsWithCommands = x))
    ...
    opt[java.time.Duration]("options.resendPhase2aAnysPeriod")
      .optionAction((x, o) => o.copy(resendPhase2aAnysPeriod = x))
    opt[Boolean]("options.useF1Optimization")
      .optionAction((x, o) => o.copy(useF1Optimization = x))
    ...
    opt[java.time.Duration]("options.leaderChangeEntryMinPeriod")
      .optionAction((x, o) => o.copy(leaderChangeEntryMinPeriod = x))
    opt[java.time.Duration]("options.leaderChangeEntryMaxPeriod")
      .optionAction((x, o) => o.copy(leaderChangeEntryMaxPeriod = x))
    ...
    opt[java.time.Duration]("options.heartbeat.failPeriod")
      .heartbeatOptionAction((x, o) => o.copy(failPeriod = x))
    opt[java.time.Duration]("options.heartbeat.successPeriod")
      .heartbeatOptionAction((x, o) => o.copy(successPeriod = x))
    opt[Int]("options.heartbeat.numRetries")
      .heartbeatOptionAction((x, o) => o.copy(numRetries = x))
    opt[Double]("options.heartbeat.networkDelayAlpha")
      .heartbeatOptionAction((x, o) => o.copy(networkDelayAlpha = x))
  }
  ...
  val server = new Server[NettyTcpTransport](
    address = config.serverAddresses(flags.index),
